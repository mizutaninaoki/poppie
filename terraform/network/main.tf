...
resource "aws_eip" "nat" {
  ...
  vpc = true
  # Elastic IPアドレスに関連付けるユーザー指定のプライマリまたはセカンダリプライベートIPアドレス。
  # プライベートIPアドレスが指定されていない場合、Elastic IPアドレスはプライマリプライベートIPアドレスに関連付けられます。
...
resource "aws_nat_gateway" "this" {
  ...
  subnet_id     = aws_subnet.publics.*.id[0] # NATゲートウェイを作成するパブリックサブネットを指定。(片方のパブリックサブネットのみ指定)
  allocation_id = aws_eip.nat.id # 先ほど作成したElastic IPをNATゲートウェイに付与する
